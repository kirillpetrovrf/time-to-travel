    ...
    _moveCacheSubscription =
      ...
      switch (moveCacheState) {
        case final MoveCacheCompleted _:
          showSnackBar(
            context,
            "Кэши перемещены в ${_settingsTextFieldController.text}",
          );
        case final MoveCacheError state:
          showSnackBar(
            context,
            "Ошибка при перемещении кэша: ${state.error}",
          );
        case _:
      }
    });
    ...
    _setCachePathSubscription =
      ...
      switch (setCachePathState) {
        case final SetCachePathSuccess _:
          showSnackBar(
            context,
            "Путь успешно установлен: ${_settingsTextFieldController.text}",
          );
        case final SetCachePathError state:
          showSnackBar(
            context,
            "Ошибка при установке пути: ${state.error}",
          );
        case _:
      }
    });
      ...
      builder: (BuildContext context) {
        return StreamBuilder(
          ...
          builder: (context, snapshot) {
            ...
            if (uiState != null) {
              print("📱 StreamBuilder: Region ${uiState.id} updated - State: ${uiState.state.name}, Progress: ${(uiState.downloadProgress * 100).toStringAsFixed(1)}%");
            }
                  ...
                  return SingleChildScrollView(
                    ...
                    child: RegionsBottomSheet(
                      ...
                      onStartDownloadButtonTapped: () {
                        print("Main: Starting download for region ${uiState.id}");
                        final isStarted =
                            _regionManager.startDownload(uiState.id);
                        print("Main: Download started: $isStarted");
                        if (!isStarted) {
                          showSnackBar(
                            context,
                            "Недостаточно свободного места на устройстве для загрузки региона с ID ${uiState.id}",
                          );
                        }
                      },
                      onStopDownloadButtonTapped: () {
                        print("Main: Stopping download for region ${uiState.id}");
                        _regionManager.stopDownload(uiState.id);
                      },
                      onPauseDownloadButtonTapped: () {
                        print("Main: Pausing download for region ${uiState.id}");
                        _regionManager.pauseDownload(uiState.id);
                      },
                      onDropDownloadButtonTapped: () {
                        print("Main: Dropping download for region ${uiState.id}");
                  ...
                  return Center(
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          Text(
                            "Данные об этом регионе не найдены :(",
