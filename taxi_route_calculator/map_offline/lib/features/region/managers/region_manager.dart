...
import 'package:map_offline/features/regions_list/managers/region_translations.dart';
...
final class RegionManager {
  ...
  Stream<RegionUiState?> uiState(int regionId) => Rx.combineLatest2(
        ...
        (region, _) => region?.let(
          (it) {
            final progress = offlineCacheManager.getProgress(region.id);
            final state = offlineCacheManager.getState(it.id);
            
            // Логирование прогресса из менеджера
            if (state == OfflineCacheRegionState.Downloading) {
              print("🏭 RegionManager: Region ${it.id} - Progress from SDK: ${(progress * 100).toStringAsFixed(2)}%, State: ${state.name}");
            }
            
            return RegionUiState(
              id: it.id,
              name: RegionTranslations.translateRegionName(it.name),
              country: it.country,
              cities: RegionTranslations.translateCityList(offlineCacheManager.getCities(it.id)),
              center: it.center,
              size: it.size.text,
