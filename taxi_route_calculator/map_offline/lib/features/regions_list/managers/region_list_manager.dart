    ...
    (regions, _, query) {
      final (completed, available) = regions
          .where((it) => {
            ...
            // Поиск по переведенному названию
            RegionTranslations.translateRegionName(it.name).toLowerCase().contains(query.toLowerCase()),
      ...
      if (completed.isNotEmpty) {
        regionsList.add(SectionItem(title: "Загружено"));
        regionsList.addAll(completed.process((offlineCacheManager.getCities)));
      }
      if (available.isNotEmpty) {
        regionsList.add(SectionItem(title: "Доступно"));
...
extension _ProcessRegions on List<OfflineCacheRegion> {
  List<RegionItem> process(List<String> Function(int id) getCities) {
    return map(
      (item) => RegionItem(
        id: item.id,
        name: RegionTranslations.translateRegionName(item.name),
        cities: RegionTranslations.translateCityList(getCities(item.id)),
      ),
    ).sortedBy((it) => it.name);
  }
}
