...
import '../features/maps/utils/map_utils.dart';
...
class _SearchFieldWithSuggestionsState extends State<SearchFieldWithSuggestions> {
  ...
  Widget _buildSuggestionItem(SuggestItem suggestion) {
    return CupertinoButton(
      padding: EdgeInsets.zero,
      onPressed: () {
        // 1️⃣ СНАЧАЛА вызываем наш callback для установки флагов
        widget.onSuggestionSelected?.call(suggestion.title.text);
        
        // 2️⃣ ПОТОМ вызываем встроенный onTap() от Яндекс API
        // Он содержит правильную логику с URI для точного поиска!
        suggestion.onTap();
        
        // 3️⃣ Скрываем overlay и убираем фокус
        _focusNode.unfocus();
        _hideOverlay();
      },
      child: Container(
  ...
  Widget build(BuildContext context) {
    ...
    return CompositedTransformTarget(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            // Кнопка выбора точки на карте с иконкой
            GestureDetector(
