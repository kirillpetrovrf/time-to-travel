...
class ThemeManager extends ChangeNotifier {
  ...
  /// Экспорт темы в JSON строку
  String exportTheme(CustomTheme theme) {
    return json.encode(theme.toJson());
  }

  /// Импорт темы из JSON строки
  CustomTheme? importTheme(String jsonString) {
    try {
      final themeData = json.decode(jsonString);
      return CustomTheme.fromJson(themeData);
    } catch (e) {
  ...
  /// Создание копии темы
  CustomTheme duplicateTheme(CustomTheme theme, String newName) {
    final timestamp = DateTime.now().millisecondsSinceEpoch;
    return theme.copyWith(id: 'custom_$timestamp', name: newName);
  }
